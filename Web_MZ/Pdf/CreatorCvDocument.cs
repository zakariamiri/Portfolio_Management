namespace Web_MZ.Pdf
{
    ...
    public class CreatorCvDocument : IDocument
    {
        private readonly ConsulterPortfolioVM _model;

        public CreatorCvDocument(ConsulterPortfolioVM model)
        {
            _model = model;
        }
        ...
        public void Compose(IDocumentContainer container)
        {
            container.Page(page =>
            {
                ...
                page.Content().Column(col =>
                {
                    ...
                    {
                        header.Item().AlignCenter().Text($"{_model.Creator.User.FirstName} {_model.Creator.User.LastName}")
                            .FontSize(22)
                            .Bold()
                        ...
                        {
                            header.Item().AlignCenter().Text(_model.Creator.Bio)
                                .Italic()
                                .FontColor(Colors.Grey.Darken2)
                                .FontSize(11);
                        }

                        header.Item().AlignCenter().Text($"📍 {_model.Creator.User.Country} | 📞 {_model.Creator.User.Phone} | ✉️ {_model.Creator.User.Email}")
                            .FontSize(10);
                    });
